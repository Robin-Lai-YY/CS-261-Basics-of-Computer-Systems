...
static int cache_line_check_validity_and_tag(cache_line_t *cache_line,
  ...
  /* TO BE COMPLETED BY THE STUDENT */
    if(cache_line->tag == tag) return cache_line->is_valid;
    else return 0;
}
...
static cache_line_t *cache_set_find_matching_line(cache_t *cache,
   ...
   */
  return NULL; // Added to remove warning.
}
...
static cache_line_t *find_available_cache_line(cache_t *cache,
  ...
  /* TO BE COMPLETED BY THE STUDENT */
  int asso = cache->associativity;
  return cache_line_make_mru(cache_set, asso-1);
...
long cache_read(cache_t *cache, long *address) {
  ...
  /* TO BE COMPLETED BY THE STUDENT */  
    ...
    if(!line || !cache_line_check_validity_and_tag(line, tag)){
        cache->miss_count++;
        line = cache_set_add(cache, set, (uintptr_t)address, (uintptr_t)tag);
    }
